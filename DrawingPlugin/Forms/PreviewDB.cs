    ...
    public class PreviewCommand
    {
        ...
        private void GenerateThumbnails(List<DatabaseBlock> blocks)
        {
            ...
            foreach (DatabaseBlock block in blocks)
            {
                try
                {
                    ...
                    // 1. Создаем новую временную БД с полной инициализацией
                    using (Database tempDb = new Database(true, true))
                    {
                        // 2. Явно создаем стандартные таблицы
                        using (Transaction tr = tempDb.TransactionManager.StartTransaction())
                        {
                            // Создаем обязательные таблицы
                            BlockTable bt = tr.GetObject(tempDb.BlockTableId, OpenMode.ForWrite) as BlockTable;
                            LayerTable lt = tr.GetObject(tempDb.LayerTableId, OpenMode.ForWrite) as LayerTable;

                            // 3. Добавляем сущности внутри транзакции временной БД
                            ...
                            {
                                // 4. Создаем сущности ВНУТРИ транзакции временной БД
                                Entity entity = CreateEntity(entityData);
                                if (entity != null)
                                {
                                    // 5. Назначаем базовый слой "0" явно
                                    entity.Layer = "0";
                                    entity.ColorIndex = 7; // Белый цвет

                                    // 6. Добавляем в модель
        ...
        private Entity CreateEntity(EntityData data)
        {
            try
            {
                ...
                if (entity != null)
                {
                    // Сбрасываем все специфические свойства
                    entity.Layer = "0";
                    entity.ColorIndex = 7;
