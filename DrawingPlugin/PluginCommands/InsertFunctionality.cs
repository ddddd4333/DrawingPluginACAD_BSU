...
namespace DrawingPlugin.PluginCommands
{
    public class InsertFunctionality()
    {
        ...
        {
            string dbPath=@"C:\Users\stas5\OneDrive\Документы\GitHub\DrawingPluginACAD_BSU\DrawingPlugin\acad_geometry.db";
        ...
        private void SaveToDatabase(Entity ent, string geometryData)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection($"Data Source={dbPath}"))
            {
                ...
                using (SQLiteCommand cmd = new SQLiteCommand(conn))
                {
                    cmd.CommandText = @"INSERT INTO entities (type, layer, color, data) VALUES (@type, @layer, @color, @data)";
