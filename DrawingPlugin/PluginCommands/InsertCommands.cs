...
namespace DrawingPlugin.PluginCommands
{
    public class InsertCommands
    {
        ...
        public bool InsertBlockAtPoint(DatabaseBlock block, Point3d insertionPoint, double scale = 1.0, double rotationAngle = 0.0)
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            ...
            try
            {
                ...
                using (Database tempDb = new Database(true, true))
                {
                    ...
                    using (Transaction tr = db.TransactionManager.StartTransaction())
                    {
                        ...
                        // Calculate the center of the block for rotation
                        ...
                        ed.WriteMessage($"\nBlock max point: {extents.Value.MaxPoint.X}, {extents.Value.MaxPoint.Y}, {extents.Value.MaxPoint.Z}");
                        ...
                        foreach (EntityData entityData in block.EntitiesData)
                        {
                            ...
                            if (entity != null)
                            {
                                ...
                                // Create a transformation matrix for rotation (convert degrees to radians)
                                double rotationRadians = rotationAngle * Math.PI / 180.0;
                                Matrix3d rotationMatrix = Matrix3d.Rotation(rotationRadians, Vector3d.ZAxis, insertionPoint);
                                
                                // Calculate the vector from the block's minimum point to the insertion point
                                Vector3d displacement = insertionPoint - extents.Value.MinPoint;
                                
                                // Apply the transformations in the correct order:
                                // 1. First translate to insertion point
                                entity.TransformBy(Matrix3d.Displacement(displacement));
                                
                                // 2. Then apply scaling
                                entity.TransformBy(scaleMatrix);
                                
                                // 3. Finally apply rotation
